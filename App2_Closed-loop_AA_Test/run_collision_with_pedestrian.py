...
def run():
    ...
    while(time.time()-time_start <= 3600 and index <= 300):        
        ...
        # collide with pedestrian
        if walker != None:
            w_location = walker.get_location()
        path = PATH([w_location.x], [w_location.y])
        ...
        # collide with pedestrian
        # graph.updatePosition(x, y, yaw, w_location.x, w_location.y, w_rotation.yaw)
        ...
        if flag:
            # adaptive attack
            observations = agent.cal_adv(observations)

            # memory attack
            # perspective_img = cv2.imread("billboard/"+str(index)+".jpg")
            # cv2.imshow("billboard", perspective_img)
            # cv2.waitKey(1)

            # stupid attack
            # closest_img = stupid_attack(v_location.x, v_location.y, v_rotation.pitch, v_rotation.yaw, v_rotation.roll, delta)
            # perspective_img = np.zeros((480, 640, 3))
            # for index in closest_img:
            #     perspective_img += cv2.imread('F:/stupid_heuristic_choice/'+dir[index])/5
            # perspective_img = perspective_img / 5.0
            # perspective_img = np.array(perspective_img, dtype=np.uint8)
            # cv2.imshow("billboard", perspective_img)
        ...
        if walker != None:
            if "J" in keys:
                walker.apply_control(carla.WalkerControl(direction=carla.Vector3D(1, 0, 0), speed = 0.1, jump = False))
                print("---------------------walker moves to left !---------------------")
            elif "K" in keys:
                walker.apply_control(carla.WalkerControl(direction=carla.Vector3D(-1, 0, 0), speed = 0.2, jump = False))
                print("---------------------walker moves to right !---------------------")
            elif "L" in keys:
                walker.apply_control(carla.WalkerControl(direction=carla.Vector3D(0, 0, 0), speed = 0, jump = False))
                print("---------------------walker stays still !---------------------")
            elif "P" in keys:
                break
        else:
            if "P" in keys:
...
if __name__ == "__main__":
    # stupid attack
    # dir = os.listdir("stupid_heuristic_choice")
    # dir_float = np.array([list(map(float, item.split('.jpg')[0].split('_'))) for item in dir])
